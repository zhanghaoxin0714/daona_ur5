...
class GravityCompensation:
    ...
    def Solve_A(self):
        A = dot(
            dot(linalg.inv(dot(self.F.transpose(), self.F)),
                self.F.transpose()), self.M
        )
    ...
    def Solve_B(self):
        B = dot(
            dot(linalg.inv(dot(self.R.transpose(), self.R)),
                self.R.transpose()), self.f
        )
    ...
    def Solve_Force(self, force_data, euler_data):
        Force_input = matrix(
            [force_data[0], force_data[1], force_data[2]]).transpose()
    ...
    def Solve_Torque(self, torque_data, euler_data):
        Torque_input = matrix(
            [torque_data[0], torque_data[1], torque_data[2]]
        ).transpose()
        ...
        Torque_ex = (
            Torque_input - Torque_zero -
            dot(dot(Gravity_param, R_array), Gravity_input)
        )
...
def main():
    #
    force_data = [-6.349214527290314e-05,
                  0.0016341784503310919, -24.31537437438965]
    torque_data = [-0.25042885541915894,
                   0.32582423090934753, 2.255179606436286e-05]
    euler_data = [-80.50866918099089,
                  77.83705434751874, -9.294185889510375 + 12]

    force_data1 = [-7.469202995300293, 2.3709897994995117, -23.0179500579834]
    torque_data1 = [-0.2169264256954193,
                    0.3719269931316376, 0.10870222747325897]
    euler_data1 = [-105.99038376663763,
                   60.89987226261212, -10.733422007074305 + 12]
    ...
    euler_data2 = [-114.24258417090118,
                   43.78913507089547, -19.384088817327235 + 12]
